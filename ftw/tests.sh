...
/ftw run -d coreruleset/tests/regression/tests --config ftw.yml --show-failures-only --read-timeout=10s --max-marker-retries=50 --cloud=$FTW_CLOUDMODE $FTW_INCLUDE || exit 1
